...
add_counts_by_pathway <- function(df_counts, df_maf, df_pathways){
  pathway_names <- sort(unique(df_pathways$Pathway_Name))
  for (pathway_name in pathway_names){
    members <- df_pathways %>% filter(Pathway_Name==pathway_name) %>% pull(Symbol)
    df_counts_pathway <- df_maf %>% filter(Hugo_Symbol %in% members) %>% 
      group_by(Tumor_Sample_Barcode, Matched_Norm_Sample_Barcode) %>%
      summarize(Count=n())
    df_counts_pathway <- df_counts_pathway %>% rename(!!pathway_name:=Count)
    df_counts <- left_join(df_counts, df_counts_pathway, how="left",
...
main <- function(args){
  ...
  df_counts <- add_counts_by_pathway(df_counts, df_maf, df_pathways)
...
if (getOption('run.main', default=TRUE)) {
  ...
  parser <- ArgumentParser(description='Build table of mutation counts per pathway.')
  ...
  parser$add_argument("--output", type="character", help="Path to output left bar data file.",
                      default="../../../results/somatic_mutations/count/count_by_pathway_sanchez_vega_DNA_all_prism.tsv")
