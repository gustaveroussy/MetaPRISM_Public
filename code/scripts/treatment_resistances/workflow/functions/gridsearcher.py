...
class GridSearchCVExtended(BaseEstimator, ClassifierMixin):
    ...
    """
    def __init__(self, grid, cv_tests=[], save_best_estimators_cv=False, save_best_estimator_train=True,
                 save_best_params=True, save_best_cv_scores=True, save_folder="models", verbose=1):
        """
        Parameters
        ----------
        grid: a GridSearchCV object
        cv_tests: a list of sklearn objects from a fold class (i.e inheriting from _BaseKFold)
        ...
        verbose: should intermediate messages be printed? the higher the value the more messages
        """
        self.grid = grid
        self.cv_tests = cv_tests
    ...
    def _compute_scores_on_cv(self, X, y, cv, suffix):
        for i, (idx_train, idx_test) in enumerate(cv.split(X,y)):
            split_name = "split%d_%s" % (i, suffix)

            if self.verbose > 0 and suffix=="grid":
                print("\nSplit [%d/%d]" % (i+1, cv.n_splits), flush=True)
                print("*"*60, flush=True)
            ...
            if hasattr(X, "iloc"):
                best_estimator = best_estimator.fit(X.iloc[idx_train], y.iloc[idx_train])
            else:
                best_estimator = best_estimator.fit(X[idx_train], y[idx_train])

            self.best_estimators_[split_name] = best_estimator

            if self.save_best_estimators_cv and suffix=="grid":
            ...
            for idx, name in zip([idx_train, idx_test], ["train", "test"]):
                row_cv_scores = {"Grid_Run": split_name, "Split": name}

                if hasattr(X, "iloc"):
                    X_idx = X.iloc[idx]
                    y_true = y.iloc[idx]
                    y_pred = best_estimator.predict(X.iloc[idx])
                else:
                    X_idx = X[idx]
                    y_true = y[idx]
                    y_pred = best_estimator.predict(X[idx])

                if self.verbose > 0 and suffix=="grid" or self.verbose > 1:
    ...
    def fit(self, X, y):
        ...
        # compute scores on grid cv and save estimators on each split if required by the user
        self._compute_scores_on_cv(X, y, cv=self.grid.cv, suffix="grid")

        # compute scores on tests cv if any
        for i, cv_test in enumerate(self.cv_tests):
            self._compute_scores_on_cv(X, y, cv=cv_test, suffix="test %d" % (i+1))

        if self.save_best_cv_scores:
            self._save_scores(self.cv_scores_, self.save_best_cv_scores)
