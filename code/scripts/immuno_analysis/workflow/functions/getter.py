...
def get_estimator(model_name):
    if model_name.startswith("MetaA"):
        return globals()[model_name.split("MetaA")[1]](estimators=[])
    elif model_name.startswith("MetaB"):
        return globals()[model_name.split("MetaB")[1]](estimators=[])
    else:
...
def _get_param_grid_meta(param_grid, cohort):
    folder = "workflow/results/mfp_models/%s" % cohort
...
def get_param_grid(model_name, grids_yaml, cohort=None):
    param_grids = yaml.load(open(grids_yaml), Loader=yaml.FullLoader)

    if model_name not in param_grids.keys():
        raise ValueError("The model %s is not in the yaml file %s." % (model_name, grids_yaml))
    else:
        param_grid = param_grids[model_name]

    if model_name.startswith("MetaA") or model_name.startswith("MetaB"):
        return _get_param_grid_meta(param_grid, cohort)
    else:
