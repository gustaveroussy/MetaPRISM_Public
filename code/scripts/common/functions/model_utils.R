...
add_col_count <- function(df_cln, col){
  col_count <- paste("Count", col, sep="_")
  df_cln[[col_count]] <- str_count(df_cln[[col]], "\\|") + 1
  df_cln <- df_cln %>% mutate(!!col_count:=ifelse(is.na(df_cln[[col_count]]), 0, .data[[col_count]]))
  df_cln
}
...
save_table <- function(table, filepath, sep="\t"){
  if (grepl(".gz$", filepath)){
    write.table(table, gsub(".gz$", "", filepath), sep=sep, quote=F, row.names=F)
    system(paste("gzip", gsub(".gz$", "", filepath)))
  } else {
    write.table(table, filepath, sep=sep, quote=F, row.names=F)
  }
  cat(paste("-filed saved at", filepath, "\n"))
}
