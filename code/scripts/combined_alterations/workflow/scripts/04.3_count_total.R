...
main <- function(args){
  ...
  df_alt <- df_alt %>% mutate(Level=ifelse(is.na(.data[[col_lvl]]), "Burden_Annotated_No_Level",
                                           paste0("Burden_Annotated_", .data[[col_lvl]])))


  # get counts total
  df_cnt <- df_alt %>% group_by(Subject_Id, Level) %>% summarize(Count=n(), .groups="drop") %>%
    spread(Level, Count)
...
if (getOption('run.main', default=TRUE)) {
  parser <- ArgumentParser(description='Build table of total alteration counts.')
